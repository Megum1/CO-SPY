...
from Datasets import RealFakeDataset as TrainDataset
...
def test_AIGC(args):
    ...
    # Begin the evaluation
    test_root = "/data4/user/cheng535/sony_intern/sony_intern_summer_2024/AIGCDetect_testset/test"
    ...
    for model_name in aigc_model_list:
        test_dataset = TrainDataset(data_path=os.path.join(test_root, model_name), split="test", transform=test_transform)
        test_loader = torch.utils.data.DataLoader(test_dataset,
                                                    batch_size=args.batch_size,
                                                    shuffle=False,
                                                    num_workers=4,
        ...
        # Save the results (update when a new test model has been evaluated)
        with open(save_result_path, "w") as f:
            json.dump(result_all, f, indent=4)
